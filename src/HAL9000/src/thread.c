    ...
    )
{
    ...
    if (pNextThread != pCurrentThread)
    {
        ...
        // 1. When the processes are different, we need to activate paging tables of the next thread's process. 
        // The context needs to be switched only when the threads come from different processes, because when they are within the same process, 
        // the threads should be scheduled fine.
