...
// -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// 1. It is not a problem if they occupy the same virtual address, because each process has its own virtual address, 
// the process thinks he owns the whole memory. The problem will appear if the same physical address will be mapped to two different processes.
// 2. 800 kb of memory is definetly better thann 400mb for the page tables.
_No_competing_thread_
STATUS
MmuInitSystem(
    ...
    )
{
    ...
    __try
    {
        // 2. a) Here is where the VA range is translated into PA.
        ...
        // 2. b) The new allocation in the kernel using the previously obtained memory descriptor list.
        // 2. c) This method should be called when setting up the user stack of a process. I think we have to map the user 
        // virtual addresses to kernel virtual addresses because some of the user applications might execute syscalls or interact with the kernel.
        // 3.Yes, the same VA is repeated for different runs of the process. I believe the result will not differ on new parallel runs.
